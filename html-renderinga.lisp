	       ...
	       :content (htmlo
			  (str
			   (list-class-instances (htmlo
						   (:img :src (make-url "/images/books.png")
							 :alt "Books")) 'topic "<ul>~{<li>~A</li>~}</ul>"))))))

(defun site-summary ()
  "<h2>Project Description</h2> 
    <p>The MultiText Project in History is an innovative educational project, brought to you by 
the History Department, University College Cork. It is the largest and most ambitious 
project undertaken by any university to provide resources for students of Modern Irish 
History at all levels: University students, the general reader, and second-level students. 
The project aims to publish a minimum of 12 books, each dealing with a separate period 
of Irish history. Each book contains accounts of key personalities, concepts, and detailed 
  ...
  (when-bind (headings (headings obj))
    (let ((midpoint (ceiling (/ (length headings) 2))))
      (flet ((print-section (list)
	       (htmlo (:td (:ul (dolist (elt list)
				  (htm (:li ((:a :href (format nil "#~A" (cadr elt)))
					     (str (car elt)))))))))))
	(htmlo ((:table :valign "top" :width "100%")
		(:tr (str (print-section (subseq headings 0 (- midpoint 1))))
		     (str (print-section (subseq headings midpoint))))))))))

(defun class-page (type title)
  (lambda ()
    (make-page title
	     :header (htmlo (:h1 (str title)))
	     :content (htmlo
			(:ul
			 (dolist (o (safe-sort
...
(defmethod render :around ((obj standard-display-mixin))
  (let* ((xrefs (lists-of-linked-objects
		 :classes (remove-if (lambda (c) (member c '(event picture topic))) *linked-classes*)
...
(define-page render-page "/display.html"
  (when.Ò%F¶W2Æ—7Böb7   them according to type, the prints a nice table"
    (let ((classes
	   (sort (copy-list
